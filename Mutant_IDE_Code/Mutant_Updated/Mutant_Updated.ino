 ...
 * 
 * NOTE: This code has been updated to use the latest Mozzi 2.0+ library
 * 
 * (C) 2021 Meebleeps
*-----------------------------------------------------------------------------------------------------------
*/
...
// Mozzi 2.0: Define control rate BEFORE including Mozzi.h
...
// MOZZI variables - kept for reference (now using MOZZI_CONTROL_RATE above)
...
#define BUTTON_INPUT_REC    4  // D10 - Records pot movements (motion recording)
#define BUTTON_INPUT_RETRIG 5  // D12 - Clones/repeats note (retrigger function)
...
void setup() 
{
  ...
  startMozzi();  // Mozzi 2.0: No parameter needed, uses MOZZI_CONTROL_RATE
}
...
void initialiseSequencer()
{
  ...
  sequencer.setMutationProbability(100);  // Mutation fully on (100%)
  sequencer.setNoteProbability(100);      // Density fully on (100%)
}
    ...
    else
    {
      switch(interfaceMode)
      {
        ...
        case INTERFACE_MODE_SHIFT:    
          ...
          // Immediately clear all parameter locks when FUNC + REC is pressed
          for (byte channel = 0; channel < MAX_PARAMETER_LOCKS; channel++)
          {
            sequencer.clearAllParameterLocks(channel);
          }
          Serial.println(F("Cleared all parameter locks"));
          break;
      }
    }
...
{
  long mixed = (long)synthVoice.updateAudio();
  mixed = (mixed * 5) >> 1;  // 2.5x volume gain
  
  // Hard limiting - prevents any clipping
  if (mixed > 255) mixed = 255;
  if (mixed < -256) mixed = -256;
  
  return MonoOutput::fromNBit(9, mixed);  // Mozzi 2.0: Proper return format
}
